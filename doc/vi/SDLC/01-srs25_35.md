...
| v1.11     | 11/05/2026 | Hồ Nguyễn Phúc Thông        | Hiệu chỉnh mục 13 Use Case chi tiết từ UC-25 đến UC-35 để đồng bộ với mục 12.2; bổ sung các use case còn thiếu về notification, đối soát job nền, promotion, notification preference và tra cứu vé khách vãng lai.                                                                                                                                          |
...
| Kích hoạt      | Admin mở chức năng cấu hình policy, commission hoặc payout                               |
| Hậu điều kiện  | Chính sách được lưu, có hiệu lực theo phạm vi / thời gian cấu hình và có audit log       |
| Ưu tiên        | Cao                                                                                      |

Luồng chính:

1. Admin mở màn hình cấu hình policy, commission hoặc payout.
2. Admin chọn nhóm cấu hình gồm thời gian giữ ghế, thời gian ngừng bán online, chính sách hủy / hoàn tiền, commission hoặc payout.
3. Admin nhập phạm vi áp dụng, thời gian hiệu lực và giá trị cấu hình.
4. Hệ thống kiểm tra dữ liệu, ngưỡng tối thiểu, trạng thái payout và xung đột với cấu hình hiện hành.
5. Admin xác nhận lưu cấu hình.
6. Hệ thống lưu chính sách mới, áp dụng cho booking phát sinh sau thời điểm hiệu lực và không áp dụng ngược cho booking cũ.
7. Hệ thống ghi audit log cho thao tác cấu hình.
...
| Kích hoạt      | Admin mở màn hình giám sát tài chính                                              |
| Hậu điều kiện  | Giao dịch được xem, đánh dấu, đối soát hoặc chuyển xử lý theo quyền               |
| Ưu tiên        | Cao                                                                               |

Luồng chính:

1. Admin mở màn hình giám sát tài chính.
2. Admin tra cứu theo booking, payment, refund, Operator hoặc provider transaction id.
3. Hệ thống hiển thị trạng thái payment, refund, escrow ledger, commission, payout và reconciliation liên quan.
4. Admin mở chi tiết giao dịch cần kiểm tra.
...
| Kích hoạt      | Admin mở hồ sơ dispute hoặc nhận yêu cầu xử lý thủ công                                  |
| Hậu điều kiện  | Dispute được cập nhật trạng thái, các bên được thông báo và quyết định xử lý được ghi audit log |
| Ưu tiên        | Cao                                                                                      |

Luồng chính:

1. Admin mở hồ sơ dispute hoặc yêu cầu xử lý thủ công.
...
| Kích hoạt      | Admin mở màn hình kiểm duyệt nội dung                                             |
...
2. Hệ thống hiển thị danh sách review, báo cáo vi phạm, banner, FAQ hoặc content page theo trạng thái.
3. Admin mở chi tiết nội dung cần kiểm duyệt.
...
| Kích hoạt      | Admin mở dashboard hoặc màn hình báo cáo toàn hệ thống                                   |
| Hậu điều kiện  | Báo cáo được hiển thị hoặc được tạo bất đồng bộ / xuất file nếu có quyền                 |
| Ưu tiên        | Trung bình                                                                               |

Luồng chính:

1. Admin mở dashboard hoặc màn hình báo cáo toàn hệ thống.
2. Admin chọn khoảng thời gian và bộ lọc theo Operator, tuyến, khu vực, phương thức thanh toán, khiếu nại hoặc audit.
...
5. Admin mở chi tiết một nhóm dữ liệu nếu có quyền.
...
| Kích hoạt      | Admin mở màn hình audit log                                                       |
| Hậu điều kiện  | Audit log được lọc, xem chi tiết hoặc xuất theo quyền                             |
| Ưu tiên        | Cao                                                                               |

Luồng chính:

1. Admin mở màn hình audit log.
2. Admin nhập điều kiện lọc theo actor, loại actor, thời gian, module, hành động, đối tượng tác động hoặc kết quả.
3. Hệ thống hiển thị danh sách audit log phù hợp.
4. Admin mở chi tiết một audit log.
...
| Kích hoạt      | Admin hoặc Nhà xe mở chức năng quản lý promotion                                  |
| Hậu điều kiện  | Promotion được tạo, cập nhật, tạm dừng, kết thúc hoặc áp dụng theo rule hợp lệ    |
| Ưu tiên        | Trung bình                                                                        |

Luồng chính:

1. Admin hoặc Nhà xe mở chức năng quản lý promotion.
2. Hệ thống xác định phạm vi quyền của actor đối với promotion cấp Platform hoặc Operator.
...
| Kích hoạt      | Actor mở màn hình cấu hình notification preference                                |
| Hậu điều kiện  | Preference được lưu và áp dụng cho các notification phù hợp                       |
| Ưu tiên        | Trung bình                                                                        |

Luồng chính:

1. Actor mở màn hình cấu hình notification preference.
2. Hệ thống hiển thị các loại thông báo và kênh gửi được phép cấu hình.
...
| Kích hoạt      | Guest mở chức năng tra cứu vé                                                            |
| Hậu điều kiện  | Thông tin vé / chuyến được hiển thị nếu xác minh hợp lệ, hoặc yêu cầu bổ sung xác minh nếu chưa đủ |
| Ưu tiên        | Trung bình                                                                               |

Luồng chính:

1. Guest mở chức năng tra cứu vé.
2. Guest nhập mã vé / mã booking và thông tin liên hệ.
3. Hệ thống kiểm tra định dạng và đối chiếu thông tin với booking hoặc ticket.
