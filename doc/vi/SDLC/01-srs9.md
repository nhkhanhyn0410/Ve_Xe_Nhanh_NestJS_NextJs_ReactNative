...
| v1.4      | 10/05/2026 | Hồ Nguyễn Phúc Thông        | Hiệu chỉnh §9 "Mô hình dữ liệu mức cao" theo cấu trúc thống nhất gồm §9.1 Thực thể chính và §9.2 Quan hệ dữ liệu chính; phân nhóm dữ liệu nghiệp vụ chính; làm rõ `Employee` thuộc nhà xe và có một trong ba role `TICKET_STAFF`, `DRIVER`, `SUPPORT_STAFF`; cập nhật quan hệ dữ liệu liên quan đến Employee, chuyến xe, vé / đặt vé, thanh toán, khiếu nại, thông báo, báo cáo / nhật trình / sự cố ở mức cao.             |
...
Mục này mô tả các nhóm dữ liệu nghiệp vụ ở mức cao, không thay thế cho thiết kế database chi tiết.

| Nhóm dữ liệu              | Thực thể                                              | Mô tả vai trò của thực thể                                                                 | Ghi chú / ràng buộc nghiệp vụ                                                             |
| ------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| Người dùng                | `User`                                                | Tài khoản hành khách sử dụng hệ thống để tìm chuyến, đặt vé, thanh toán và quản lý lịch sử | Người dùng chỉ được truy cập booking, ticket, đánh giá và khiếu nại của chính mình         |
| Quản trị hệ thống         | `Admin`                                               | Tài khoản quản trị toàn hệ thống                                                           | Thao tác nhạy cảm của Admin phải được phân quyền và ghi nhận vào audit log                 |
| Nhà xe                    | `Operator`                                            | Nhà xe / doanh nghiệp vận tải cung cấp dịch vụ vận chuyển trên nền tảng                    | Là tenant nghiệp vụ chính; dữ liệu vận hành của từng nhà xe phải được cô lập               |
| Nhân viên nhà xe          | `Employee`                                            | Tài khoản nhân sự thuộc một `Operator`, do nhà xe tạo và quản lý trong Operator OS         | Mỗi Employee thuộc đúng một nhà xe và có một role chính: `TICKET_STAFF`, `DRIVER`, `SUPPORT_STAFF` |
| Xe                        | `Bus`, `BusType`                                      | Phương tiện vận tải và loại xe như ghế ngồi, giường nằm, limousine, cabin                  | Xe thuộc nhà xe và được sử dụng để vận hành các chuyến cụ thể                             |
| Ghế / sơ đồ ghế           | `SeatMap`, `Seat`                                     | Sơ đồ bố trí ghế / giường và từng vị trí có thể bán vé trên xe                             | Là cơ sở để chọn ghế, giữ ghế và chống bán trùng ghế trên cùng một chuyến                 |
| Tuyến đường               | `Route`, `StopPoint`                                  | Tuyến vận tải và các điểm đón / trả / dừng liên quan                                       | Tuyến và điểm dừng phải đủ dữ liệu để phục vụ tìm kiếm, hiển thị và lập chuyến             |
| Chuyến xe                 | `Trip`, `TripStop`, `Fare`                            | Chuyến xe cụ thể theo ngày giờ, lộ trình, xe, điểm dừng, trạng thái và giá vé áp dụng      | Chuyến chỉ được mở bán khi có tuyến, xe, sơ đồ ghế và giá vé hợp lệ                       |
| Vé / đặt vé               | `Booking`, `Ticket`                                   | Đơn đặt vé và vé điện tử cho từng ghế / hành khách trên chuyến                             | Vé phải gắn với booking, chuyến, ghế, điểm đón / trả và có mã / QR duy nhất                |
| Thanh toán                | `Payment`, `Refund`                                   | Giao dịch thanh toán, đối soát và hoàn tiền liên quan đến booking                          | Giao dịch tài chính phải truy vết được từ booking, payment đến refund                     |
| Khuyến mãi                | `Promotion`                                           | Mã giảm giá hoặc chương trình khuyến mãi áp dụng khi đặt vé                                | Điều kiện áp dụng phải rõ ràng và được phản ánh trong tổng tiền booking                    |
| Đánh giá / khiếu nại      | `Review`, `Complaint`                                 | Đánh giá chuyến đi / nhà xe và yêu cầu hỗ trợ hoặc khiếu nại của hành khách                | Khiếu nại có thể liên quan đến vé, thanh toán, chất lượng dịch vụ, chuyến đi hoặc nhà xe   |
| Thông báo                 | `Notification`                                        | Thông báo gửi tới User, Operator, Employee hoặc Admin                                      | Dùng cho sự kiện đặt vé, thanh toán, đổi / hủy chuyến, hoàn tiền, khiếu nại và vận hành    |
| Báo cáo / nhật trình / sự cố | `JourneyLog`, `IncidentReport`, `OperationReport`, `AuditLog` | Dữ liệu nhật trình chuyến, báo cáo vận hành, báo cáo sự cố và nhật ký thao tác hệ thống    | Là bằng chứng phục vụ vận hành, đối soát, kiểm duyệt, xử lý tranh chấp và audit            |

### 9.2. Quan hệ dữ liệu chính

- Một `Operator` có nhiều `Bus`, `SeatMap`, `Route`, `Trip`, `Fare`, `Promotion` và `Employee`; đồng thời sở hữu dữ liệu booking, ticket, thanh toán, đánh giá, khiếu nại và báo cáo phát sinh trong phạm vi nhà xe.
- Một `Employee` luôn thuộc một `Operator` và được nhà xe gán một trong ba role:
  - `TICKET_STAFF`: phụ trách nghiệp vụ vé, đặt vé, xác nhận hành khách hoặc hỗ trợ bán vé theo phân quyền.
  - `DRIVER`: được phân công cho chuyến, cập nhật trạng thái chuyến và ghi nhận nhật trình / sự cố trong quá trình vận hành.
  - `SUPPORT_STAFF`: hỗ trợ xử lý sự cố, khiếu nại hoặc chăm sóc khách hàng trong phạm vi nhà xe.
- Một `Bus` thuộc một `Operator`, có `SeatMap` và nhiều `Seat`; dữ liệu ghế được dùng khi mở bán, giữ ghế, phát hành vé và kiểm soát không bán trùng ghế trên cùng một chuyến.
- Một `Route` gồm các `StopPoint` theo thứ tự nghiệp vụ; một `Trip` được tạo từ một `Route` và có thể có `TripStop` để thể hiện điểm dừng / điểm đón / điểm trả áp dụng cho chuyến cụ thể.
- Một `Trip` thuộc một `Operator`, sử dụng một `Bus`, áp dụng `Fare` phù hợp và có thể gán các `Employee` theo phạm vi công việc. `DRIVER` là role chịu trách nhiệm chính cho vận hành chuyến và nhật trình chuyến.
- Một `User` có thể tạo nhiều `Booking`; mỗi `Booking` có một hoặc nhiều `Ticket`, và mỗi `Ticket` gắn với một `Trip`, một `Seat`, thông tin hành khách, điểm đón và điểm trả.
- Một `Booking` có thể áp dụng `Promotion` nếu thỏa điều kiện; tổng tiền booking phải thể hiện rõ giá vé, giảm giá, phí và số tiền cần thanh toán.
- Một `Booking` có một hoặc nhiều `Payment` và có thể phát sinh `Refund` theo chính sách hủy / hoàn tiền. Trạng thái thanh toán, hoàn tiền và đối soát phải truy vết được bằng mã giao dịch.
- Một `User` có thể tạo `Review` hoặc `Complaint` liên quan đến booking, ticket, trip hoặc nhà xe. `SUPPORT_STAFF`, `Operator` và `Admin` xử lý theo phạm vi trách nhiệm và phân quyền.
- `Notification` được liên kết với actor nhận thông báo và sự kiện nghiệp vụ tương ứng như đặt vé, thanh toán, đổi / hủy chuyến, hoàn tiền, phân công nhân viên, khiếu nại hoặc sự cố.
- `JourneyLog`, `IncidentReport` và `OperationReport` gắn với `Trip`, `Employee` và `Operator`; `AuditLog` ghi nhận thao tác nhạy cảm của User, Operator, Employee hoặc Admin để phục vụ truy vết và kiểm duyệt.
