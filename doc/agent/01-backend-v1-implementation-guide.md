...
| Gate | Cần đạt |
| ---- | ------- |
| START-G1 | `PROJECT-STATE` xác nhận 01..05, 07, 08 đủ baseline. |
| START-G2 | Reviewer xác nhận hoặc risk-accept ADR-010/011 nếu dùng NestJS + Redis/Bull-compatible queue. |
| START-G3 | Runtime/package manager/Docker image được pin cho CI. |
| START-G4 | Có plan/owner cho OpenAPI, DB index manifest, seed, provider mock. |
| START-G5 | Local/CI DB hỗ trợ MongoDB transaction. |
