...
- [11. Chi tiết các module backend](#11-chi-tiết-các-module-backend)
- [12. Tính năng tìm kiếm và định tuyến](#12-tính-năng-tìm-kiếm-và-định-tuyến)
- [13. Cấu hình Docker và OSRM](#13-cấu-hình-docker-và-osrm)
- [14. Testing và Quality Assurance](#14-testing-và-quality-assurance)
- [15. Hướng dẫn đóng góp](#15-hướng-dẫn-đóng-góp)
...
- **Mobile**: Ứng dụng React Native + Expo cho trải nghiệm di động.
...
- **Cache/Queue**: Redis để cache và Bull để xử lý hàng đợi công việc.
- **Routing**: OSRM (Open Source Routing Machine) để tính toán lộ trình và thời gian di chuyển.
- **Dịch vụ ngoài**: Mongo Express cho quản trị MongoDB, Redis Commander cho Redis.

## 5. Công nghệ sử dụng

- **Ngôn ngữ**: `TypeScript`
- **Frontend**: `Next.js`, `React`, `Ant Design`, `Tailwind CSS`, `Zustand`, `React Query`, `React Hook Form`
- **Backend**: `NestJS`, `Mongoose`, `Passport`, `JWT`, `Bull`, `Socket.IO`, `Winston`, `Helmet`, `Swagger`
- **Mobile**: `React Native`, `Expo`, `Expo Router`
- **Cơ sở dữ liệu**: `MongoDB`
- **Cache/hàng đợi**: `Redis`, `Bull`
- **Định tuyến bản đồ**: `OSRM`
- **Công cụ phát triển**: `ESLint`, `Prettier`, `Jest`, `Docker Compose`, `TypeScript`
...
- `/apps/frontend`: mã nguồn frontend Next.js.
  - `src/app/`: phần giao diện chính sử dụng App Router.
- `/apps/mobile`: mã nguồn mobile React Native + Expo.
  - `src/app/`: routing với Expo Router.
  - `src/lib/`: utilities và providers.
  - `src/store/`: state management với Zustand.
- `/packages/shared-types`: thư viện chia sẻ kiểu TypeScript giữa frontend và backend.
- `/packages/api-client`: client API tự động generate cho frontend/mobile.
...
- Dự án sử dụng cấu hình môi trường trong backend; kiểm tra file `.env` nếu có trong `apps/backend` để thiết lập thông số MongoDB, JWT, Redis, OSRM.
- Các biến môi trường cần thiết:
  - `MONGODB_URI`: URI kết nối MongoDB
  - `JWT_SECRET`: Secret key cho JWT
  - `REDIS_URL`: URL Redis
  - `OSRM_URL`: URL OSRM service
  - `ALLOWED_ORIGINS`: Danh sách origins cho CORS
...
npm run dev:backend    # Backend NestJS
...
```bash
# Khởi động tất cả services (MongoDB, Redis, OSRM, Mongo Express, Redis Commander)
npm run docker:dev
...
### 8.5 Chuẩn bị dữ liệu OSRM

OSRM yêu cầu dữ liệu bản đồ Việt Nam để tính toán lộ trình:

```bash
cd docker/osrm
./prepare-data.sh  # Tải và xử lý dữ liệu OSM (~100MB, mất 5-10 phút)
```

Sau đó khởi động OSRM:
...
## 11. Chi tiết các module backend

Backend được tổ chức theo module pattern của NestJS:

- **Auth**: Xác thực người dùng với JWT và Passport.
- **Users**: Quản lý thông tin người dùng và profile.
- **Admin**: Chức năng quản trị hệ thống với phân quyền.
- **Operators**: Quản lý nhà xe và operator.
- **StopPoints**: Điểm dừng trên tuyến đường.
- **Routes**: Tuyến đường và lộ trình.
- **Buses**: Thông tin xe buýt và tiện nghi.
- **Trips**: Chuyến đi và lịch trình.
- **Employees**: Nhân viên và tài xế.
- **Bookings**: Đặt vé và quản lý booking.
- **Search**: Tìm kiếm chuyến đi với nhiều strategies.
- **Redis**: Cache và queue management.
- **OSRM**: Tích hợp với Open Source Routing Machine.

## 12. Tính năng tìm kiếm và định tuyến

Hệ thống tìm kiếm sử dụng 3 tầng strategy:

- **GeoSearchStrategy**: Tìm điểm dừng gần nhất dựa trên vị trí GPS.
- **SubRouteStrategy**: Tìm chuyến trong tuyến con.
- **TransferMatchStrategy**: Tìm chuyến nối với trung chuyển.

Các loại hành trình (JourneyType):

- **DIRECT**: Chuyến trực tiếp.
- **WITH_TRANSIT**: Chuyến có trung chuyển đưa đón.
- **TRANSFER**: Nối chuyến qua điểm trung chuyển.
- **TRANSIT_AND_TRANSFER**: Kết hợp trung chuyển và nối chuyến.

OSRM được sử dụng để tính toán thời gian và khoảng cách thực tế giữa các điểm.

## 13. Cấu hình Docker và OSRM

Docker Compose cung cấp:

- **MongoDB**: Cơ sở dữ liệu chính.
- **Redis**: Cache và message queue.
- **OSRM**: Routing engine với dữ liệu Việt Nam.
- **Mongo Express**: GUI quản trị MongoDB (http://localhost:8081).
- **Redis Commander**: GUI quản trị Redis (http://localhost:8083).

OSRM sử dụng dữ liệu OSM Việt Nam từ Geofabrik, xử lý qua các bước: extract, partition, customize để tối ưu cho routing.

## 14. Testing và Quality Assurance

```bash
# Chạy test cho tất cả apps
npm run test
...
Backend sử dụng Jest cho unit test và e2e test. Swagger UI available tại `/api/docs`.

## 15. Hướng dẫn đóng góp
