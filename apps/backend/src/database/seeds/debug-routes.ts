...
async function debug(): Promise<void> {
  ...
  for (const r of routes) {
    ...
    if (r.stops && Array.isArray(r.stops)) {
      for (const s of r.stops) {
        console.log(
          `    [order=${s.order}] role=${s.role ?? 'N/A'} stopPointId=${s.stopPointId ?? 'N/A'} arrMin=${s.estimatedArrivalMinutes}`,
        );
      }
    }
    // Check old format
    if (r.originId)
      console.log(
        `    OLD FORMAT -> originId=${r.originId}, destinationId=${r.destinationId}`,
      );
  }
  ...
  const trips = await db
    .collection('trips')
    .find({ departureTime: { $gte: new Date() } })
    .project({
      routeId: 1,
      departureTime: 1,
      arrivalTime: 1,
  ...
  for (const t of trips) {
    ...
    console.log(
      `  ${route?.routeCode ?? '???'} | ${t.departureTime?.toISOString()} -> ${t.arrivalTime?.toISOString()} | ${t.finalPrice} VND | ${t.status}`,
    );
  }
